...
interface ContractDisplayProps {
    ...
    onUpdateSections: React.Dispatch<React.SetStateAction<ContractSection[]>>;
      ...
      while (true) {
        ...
        for (const event of events) {
          try {
            ...
            if (data.type === 'report') {
              onUpdateSections((prevSections: ContractSection[]) => 
            ...
            } else if (data.type === 'redline') {
              originalContent += data.originalContract;
              modifiedContent += data.modifiedContract;
              onUpdateSections((prevSections: ContractSection[]) => 
                prevSections.map(section => 
                  section.id === newSectionId
                    ? { ...section, originalContent: originalContent, content: modifiedContent }
                    : section
                )
              );
              ...
              <Card className="pt-10 bg-transparent relative">
                ...
                {section.content ? (
                    section.originalContent ? (
                        ...
                        {diffWords(section.originalContent, section.content).map((part, index) => {
                            let className = '';
                            if (part.added) {
                        ...
                        // If no originalContent, render content as usual
                        <div
