...
export async function POST(req: NextRequest) {
  ...
  try {
    const { prompt, filePath, favoriteTheme } = await req.json();
    console.log('Received prompt:', prompt, 'filePath:', filePath, 'favoriteTheme:', favoriteTheme);
    if (!prompt || !filePath) {
      console.log('Missing required parameters');
      return NextResponse.json({ error: 'Prompt and file path are required' }, { status: 400 });
    }
    ...
    const response = await fetch(`${pythonServerUrl}/generate-powerpoint`, {
      ...
      body: JSON.stringify({ prompt, filePath, favoriteTheme }),
    });
    if (!response.ok) {
